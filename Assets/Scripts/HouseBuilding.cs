	...
	void Update () {
		if (!builtHouse && (Time.time - t) > 15f) {
			...
			if (h == HouseType.Straw)
				Object.Instantiate ((Resources.Load ("Houses/StrawHousePrefab", typeof(GameObject)) as GameObject), (transform.position - new Vector3(0,6,0)), (Resources.Load ("Houses/StrawHousePrefab", typeof(GameObject)) as GameObject).transform.rotation);
			if (h == HouseType.Bricks)
				Object.Instantiate ((Resources.Load ("Houses/BrickHousePrefab", typeof(GameObject)) as GameObject), (transform.position + new Vector3(0,1,0)), Quaternion.identity);
			builtHouse = true;
		}
		if (!builtHouse) {
